...
/// A defensible stop, not successful acceptance. Keep this separate from the
/// quality predicates so stopping never inflates the saved assessment.
pub fn goal_has_evidenced_acceptance_blocker(goal: &TodoGoal) -> bool {
    goal.feedback_loop_relevance == Some(FeedbackLoopRelevance::AcceptanceBlocked)
        && goal.iteration_maturity == Some(IterationMaturity::ConstraintsExhausted)
        && goal
            .feedback_loop
            .as_deref()
...
pub fn build_todo_ownership_continuation_message(todos: &[TodoItem], goals: &[TodoGoal]) -> String {
    ...
    for group in groups {
        ...
        if goal_has_evidenced_acceptance_blocker(goal) {
            continue;
        }
        if !goal
            .delivery_state
...
/// A terminal handoff that preserves the distinction between blocked and passed.
pub const TODO_BLOCKED_RESPONSE_CONTINUATION_MESSAGE: &str = "[auto] Further acceptance is blocked by a documented constraint. Give the user a concise final response with the observed blocker, useful partial result, and condition needed to resume. Do not repeat unchanged checks or claim acceptance passed.";
...
pub fn build_gate_digest(
    ...
    for observation in observations {
        if observation.kind != GateObservationKind::IntentUnderstanding
            && goals.iter().any(|goal| {
                normalized_group(goal.group.as_deref())
                    == normalized_group(observation.group.as_deref())
                    && goal_has_evidenced_acceptance_blocker(goal)
            })
        {
            continue;
        }
...
pub fn completed_groups_have_sufficient_delivery(todos: &[TodoItem], goals: &[TodoGoal]) -> bool {
    completed_groups_satisfy(todos, goals, delivery_state_passes)
}

/// Automatic continuation may stop at an evidenced acceptance blocker without
/// claiming that the completed groups passed their delivery checks.
pub fn completed_groups_can_stop(todos: &[TodoItem], goals: &[TodoGoal]) -> bool {
    completed_groups_satisfy(todos, goals, |goal| {
        delivery_state_passes(goal) || goal_has_evidenced_acceptance_blocker(goal)
    })
}

fn completed_groups_satisfy(
    todos: &[TodoItem],
    goals: &[TodoGoal],
    predicate: impl Fn(&TodoGoal) -> bool,
) -> bool {
    let mut groups: Vec<Option<String>> = Vec::new();
    for todo in todos {
        let group = normalized_group(todo.group.as_deref());
...
pub fn is_auto_poke_message(message: &str) -> bool {
    ...
    (trimmed.starts_with("You have ")
        ...
        || trimmed.starts_with(TODO_BLOCKED_RESPONSE_CONTINUATION_MESSAGE)
...
pub fn auto_poke_display_summary(message: &str) -> Option<&'static str> {
    ...
    if trimmed.starts_with(TODO_BLOCKED_RESPONSE_CONTINUATION_MESSAGE) {
        return Some("Acceptance is blocked. Preparing the remaining constraints...");
    }
