...
- If git is dirty or other agents work here in parallel, still try to commit your work.
...
- **Rebuild when done** - Build the source after your changes.
- **Bump version for releases** - Update the version in `Cargo.toml` for releases. Review all changes since the last release to choose the bump: patch, minor, etc.
- **Remote builds available** - Use `scripts/remote_build.sh` to offload heavy cargo work to another machine. A terminated build likely means insufficient local resources. Use remote build then. Try checking machine resources before building.

## Logs
- Logs go to `~/.jcode/logs/` (daily files like `jcode-YYYY-MM-DD.log`).

## Debug Socket
- Use the debug socket for runtime debugging
...
- `~/.jcode/builds/canary/jcode` remains for canary/testing, not the primary self-dev install path.
